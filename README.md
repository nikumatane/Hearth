...
管理员与新建/修改的成员密码最低为 10 个字符；这是最低可用限制，仍建议使用 14 个
字符以上的唯一密码或长短语。
...
总览和游戏详情页看到其有权发起的当前任务阶段与进度。任务历史使用位于 Hearth 状态目录的
轻量索引跨面板重启保留，默认保留最近 100 条且最长 30 天；记录到期时同步删除其关联的
Hearth 任务日志。面板日志常驻任务日志页；每次启动、更新或版本检查生成的日志与对应操作
记录关联，点击时才读取，运行中的任务会自动显示且任务标签可以关闭。当前选中的日志会在
日志页快速刷新，游戏启动任务完成后仍可继续跟随其控制台输出。
...
可以在登录记录右侧三点菜单将该次登录来源 IP 快速加入黑名单 24 小时或白名单 7 天，也可以在
“IP 黑白名单”页设置 1 小时至 365 天或永久规则。黑名单在密码计算前拒绝请求；白名单
仍必须输入正确密码。规则与成员凭据变更只进入“安全操作审计”，不会伪装成登录记录。
...
详情页在线玩家列表只展示经过清理的游戏昵称，不通过面板 API 返回平台账号、Player ID
或 User ID。玩家上限优先取运行中 REST metrics 的实际值；REST 不可用且当前世界存在
`WorldOption.sav` 时显示“上限未知”，不会错误使用可能已被 WorldOption 接管的 INI 值。
安全关闭前会再次查询在线人数：确认无人在线时把通知等待缩短到 5 秒；有人在线或无法
确认人数时仍使用 `shutdownWaitSeconds`（默认 30 秒），任务详情会显示关闭倒计时。
...
Palworld 详情页在“当前版本”旁显示服务端版本检查状态。Hearth 启动 30 秒后尝试首次
检查，之后每 15 分钟判断一次；距离最近一次成功检查满 6 小时才会再次执行。也可以点击
“检查新版本”立即触发。失败自动重试至少间隔 1 小时。检查仍是普通串行任务，不会抢占
其他 Hearth 或 SteamCMD 任务。

检查分为两个内部阶段：先让 SteamCMD 完成自身准备，再单独刷新 App ID `2394010` 的
public 分支信息。SteamCMD 自身版本既不展示，也不参与服务端更新结论；页面只显示
Palworld 服务端是否有可用更新。手动检查需要“更新服务端”权限。结果只缓存在当前 Hearth
进程中，面板重启或本机服务端 Build ID 变化后重新检查。更新结论比较本机已安装 depot
manifest 与 public 分支对应 depot manifest，而不是 App 顶层 Build ID，因此不会把只影响
其他 depot 或应用元数据的变化误报为 Palworld 服务端更新，且不依赖第三方版本站点。
