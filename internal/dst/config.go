...
var dstConfigFiles = []dstConfigFileDefinition{
	{id: "cluster", name: "cluster.ini", path: func(dir string) string { return filepath.Join(dir, "cluster.ini") }},
	{id: "master", name: "Master/server.ini", path: func(dir string) string { return filepath.Join(dir, "Master", "server.ini") }},
	{id: "caves", name: "Caves/server.ini", path: func(dir string) string { return filepath.Join(dir, "Caves", "server.ini") }},
}
...
func (s *Service) writeDSTConfigLocked(current panel.DSTConfigDocument, files map[string]string) (panel.DSTConfigDocument, error) {
	...
	for id, content := range files {
		...
		if err := validateDSTINI(content); err != nil {
			return panel.DSTConfigDocument{}, fmt.Errorf("%w: %s 格式无效: %v", panel.ErrInvalid, definition.name, err)
		}
		ids = append(ids, id)
	}
	sort.Strings(ids)
