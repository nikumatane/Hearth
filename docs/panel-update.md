...
面板更新器只消费公开仓库的 GitHub Release，不读取 GitHub Token。要让测试包出现在面板中：

1. 用目标版本创建公开 prerelease（例如 `v1.3.0-rc.1`），并上传精确命名的
   `hearth-windows-amd64-v1.3.0-rc.1.zip` 与同名 `.sha256`；ZIP 内 `VERSION` 必须完全一致。
2. 在“系统设置 → 后台设置”选择 `Prerelease`，保存并按提示重启 Hearth。
3. 打开“系统设置 → 面板更新”，点击“检查更新”，确认版本和校验信息后再点击“准备更新”。

只有本地分支或未发布的 ZIP 时，面板不会直接读取本地文件；请先用安装脚本/远程复制手动安装，
或将该包发布为 prerelease。发布前先在测试机验证，生产机保持 Stable。
